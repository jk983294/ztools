...
void merge_read(zerg::InputData& m_id, const nlohmann::json& config) {
  ...
  for (auto& entry : entries) {
    ...
    int start_col_idx = feat_names.size();
    ...
    for (int32_t date : trading_dates) {
      ...
      if (first_date) {
        int end_col_idx = feat_names.size();
        if (end_col_idx > start_col_idx) {
          int middle_col = (end_col_idx + start_col_idx) / 2;
          ZLOG("add %d cols [%s...%s...%s] from %s", end_col_idx - start_col_idx,
            feat_names[start_col_idx].c_str(), feat_names[middle_col].c_str(),
            feat_names[end_col_idx - 1].c_str(), dir.c_str());
        } else {
          ZLOG("WARN, no col in %s", dir.c_str());
        }
      }
